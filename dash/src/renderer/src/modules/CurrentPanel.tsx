import { useSignal } from '../store/signals';

const MAX_CURRENT = 60;

// ── Gauge geometry ──────────────────────────────────────────────────

const S_CUR: number = 1;

const CX = 200;
const CY = 220;
const R = 160;
const START_DEG = 240;
const SWEEP_DEG = 240;

function polarToCartesian(
  cx: number,
  cy: number,
  r: number,
  angleDeg: number,
): { x: number; y: number } {
  const rad = (angleDeg * Math.PI) / 180;
  return { x: cx + r * Math.sin(rad), y: cy - r * Math.cos(rad) };
}

function describeArc(cx: number, cy: number, r: number, startDeg: number, endDeg: number): string {
  const s = polarToCartesian(cx, cy, r, startDeg);
  const e = polarToCartesian(cx, cy, r, endDeg);
  const large = endDeg - startDeg > 180 ? 1 : 0;
  return `M ${s.x.toFixed(1)} ${s.y.toFixed(1)} A ${r} ${r} 0 ${large} 1 ${e.x.toFixed(1)} ${e.y.toFixed(1)}`;
}

// ── Tick helpers ────────────────────────────────────────────────────

interface Tick {
  x1: number;
  y1: number;
  x2: number;
  y2: number;
  labelX: number;
  labelY: number;
}

function computeTick(deg: number): Tick {
  const inner = polarToCartesian(CX, CY, R - 12, deg);
  const outer = polarToCartesian(CX, CY, R, deg);
  const labelPos = polarToCartesian(CX, CY, R - 26, deg);
  return {
    x1: inner.x,
    y1: inner.y,
    x2: outer.x,
    y2: outer.y,
    labelX: labelPos.x,
    labelY: labelPos.y,
  };
}

const CUR_MAJOR: Tick[] = [];
const CUR_MINOR: Tick[] = [];
for (let a = 0; a <= MAX_CURRENT; a += 5) {
  const frac = a / MAX_CURRENT;
  const deg = START_DEG + frac * SWEEP_DEG;
  const t = computeTick(deg);
  if (a % 10 === 0) {
    CUR_MAJOR.push(t);
  } else {
    CUR_MINOR.push(t);
  }
}

// ── Component ───────────────────────────────────────────────────────

export function CurrentPanel() {
  const raw = useSignal('acu_accumulator_current');
  const clamped = Math.max(0, Math.min(MAX_CURRENT, Math.abs(raw)));
  const frac = clamped / MAX_CURRENT;
  const needleDeg = START_DEG + frac * SWEEP_DEG;

  const bgArc = describeArc(CX, CY, R, START_DEG, START_DEG + SWEEP_DEG);
  const fgArc = frac > 0 ? describeArc(CX, CY, R, START_DEG, needleDeg) : '';

  const needleInnerR = 48;
  const needleOuterR = R - 35;
  const needleInner = polarToCartesian(CX, CY, needleInnerR, needleDeg);
  const needleOuter = polarToCartesian(CX, CY, needleOuterR, needleDeg);

  return (
    <div className="relative flex flex-col items-center justify-center gap-1 rounded-2xl border border-neutral-800 bg-gradient-to-b from-neutral-900/80 to-neutral-900/40 px-4 py-3">
      {/* Brand glow */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(52,211,153,0.08),rgba(34,211,238,0.04)_55%,transparent_75%)]" />

      <div
        className="flex flex-col items-center gap-1"
        style={{ transform: `scale(${S_CUR})`, transformOrigin: 'center center' }}
      >
        <svg viewBox="0 0 400 400" className="w-full max-w-[400px]">
          {/* Background arc */}
          <path
            d={bgArc}
            fill="none"
            stroke="rgb(39 39 42)"
            strokeWidth={10}
            strokeLinecap="round"
          />

          {/* Foreground arc */}
          {fgArc && (
            <path
              d={fgArc}
              fill="none"
              stroke="url(#curGrad)"
              strokeWidth={10}
              strokeLinecap="round"
              className="transition-all duration-150 ease-out"
            />
          )}

          <defs>
            <linearGradient id="curGrad" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" stopColor="rgb(52 211 153)" />
              <stop offset="50%" stopColor="rgb(34 211 238)" />
              <stop offset="100%" stopColor="rgb(56 189 248)" />
            </linearGradient>
          </defs>

          {/* Minor ticks */}
          {CUR_MINOR.map((t, i) => (
            <line
              key={`mi${i}`}
              x1={t.x1}
              y1={t.y1}
              x2={t.x2}
              y2={t.y2}
              stroke="rgb(63 63 70)"
              strokeWidth={2}
              strokeLinecap="round"
            />
          ))}

          {/* Major ticks + labels */}
          {CUR_MAJOR.map((t, i) => {
            const labelVal = i * 10;
            return (
              <g key={`ma${i}`}>
                <line
                  x1={t.x1}
                  y1={t.y1}
                  x2={t.x2}
                  y2={t.y2}
                  stroke="rgb(113 113 122)"
                  strokeWidth={3}
                  strokeLinecap="round"
                />
                <text
                  x={t.labelX}
                  y={t.labelY + 4}
                  fill="rgb(113 113 122)"
                  fontSize={12}
                  fontFamily="ui-sans-serif, system-ui, sans-serif"
                  textAnchor="middle"
                  dominantBaseline="central"
                  className="font-bold tabular-nums"
                >
                  {labelVal}
                </text>
              </g>
            );
          })}

          {/* Needle */}
          <line
            x1={needleInner.x}
            y1={needleInner.y}
            x2={needleOuter.x}
            y2={needleOuter.y}
            stroke="rgb(34 211 238)"
            strokeWidth={3}
            strokeLinecap="round"
            className="transition-all duration-150 ease-out"
          />

          {/* Current in center */}
          <text
            x={CX}
            y={CY - 6}
            fill="rgb(250 250 250)"
            fontSize={56}
            fontFamily="ui-sans-serif, system-ui, sans-serif"
            textAnchor="middle"
            dominantBaseline="central"
            className="font-black tabular-nums"
          >
            {Math.round(clamped)}
          </text>
          <text
            x={CX}
            y={CY + 34}
            fill="rgb(113 113 122)"
            fontSize={14}
            fontFamily="ui-sans-serif, system-ui, sans-serif"
            textAnchor="middle"
            dominantBaseline="central"
            className="font-semibold tracking-[0.5em]"
          >
            AMPS
          </text>
        </svg>
      </div>
    </div>
  );
}
